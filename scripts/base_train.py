...
parser.add_argument("--wandb-every", type=int, default=1, help="log training metrics to wandb every N steps (1 = every step)")
parser.add_argument("--wandb-project", type=str, default="nano-moe", help="wandb project name")
# Runtime
...
if orig_model.is_moe():
    ...
    print0(f"MoE: {args.n_expert} experts, top-{args.top_k}, Quantile Balancing (hyperparameter-free)")
    print0(f"Matmul params: {total_matmul:,} total / {active_matmul:,} active ({active_matmul / total_matmul:.1%})")
    print0(f"All params:    {num_params:,} total / {num_params - (total_matmul - active_matmul):,} active")
    print0("NOTE: batch-size/LR/weight-decay scaling below is inherited from dense nanochat's")
    print0("      scaling-law fits and has NOT been re-derived for MoE. See README limitations.")
...
while True:
    ...
    if scaler is not None:
        ...
        optimizer.step()
    # Quantile Balancing: betas accumulated across this step's micro-batches become the router
    # bias for the next step (the reference pipelines them through the training state the same
    # way). No-op when dense. Cheap: one topk per MoE layer per step, no extra forward.
    ...
    # aux-loss-free literature. 0.0 is perfectly balanced; it is the headline QB is judged on.
    if args.expert_load_every > 0 and orig_model.is_moe() and step % args.expert_load_every == 0:
        counts = orig_model.expert_load(reset=True).float()  # (n_moe_layer, n_expert)
        if counts.sum() > 0:
            mean_load = counts.mean(dim=-1, keepdim=True)
            maxvio = ((counts.max(dim=-1).values - mean_load.squeeze(-1)) / mean_load.squeeze(-1))
