...
class MoEMLP(nn.Module):
    ...
    def _dispatch_grouped(self, xf, idx, gate):
        ...
        flat = idx.reshape(-1)
        order = torch.argsort(flat, stable=True)          # group pairs by expert
...
class Block(nn.Module):
    def __init__(self, config, layer_idx):
        ...
        self.mlp = MoEMLP(config) if config.n_expert > 1 else MLP(config)
...
class GPT(nn.Module):
    ...
    def expert_load(self, reset=True):
        ...
        for block in self.transformer.h:
            if isinstance(block.mlp, MoEMLP):
                counts.append(block.mlp.expert_counts.clone())
                if reset:
                    block.mlp.expert_counts.zero_()
